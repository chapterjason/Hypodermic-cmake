...
#include "AutowireableConstructor.hpp"
#include "Behavior.hpp"
#include "ConstructorDescriptor.hpp"
...
#include "NestedRegistrationScope.hpp"
#include "ResolutionContext.hpp"
#include "ResolutionException.hpp"
    ...
    class ComponentContext {
        public:
            ComponentContext(const std::shared_ptr<Container> &container,
                    ...
                    : m_registrationScope(registrationScope), m_runtimeRegistrationBuilder(runtimeRegistrationBuilder),
        ...
        private:
            ...
            template<class T>
            std::shared_ptr<T>
            resolve(const TypeAliasKey &typeAliasKey, const std::shared_ptr<RegistrationContext> &registrationContext) {
                return std::static_pointer_cast<T>(resolveErasedType(typeAliasKey, registrationContext));
            }

            std::shared_ptr<void> resolveErasedType(const TypeAliasKey &typeAliasKey,
                                                    const std::shared_ptr<RegistrationContext> &registrationContext) {
                auto &resolutionContainer = registrationContext->resolutionContainer();
                return resolutionContainer.getOrCreateComponent(typeAliasKey, registrationContext->registration(),
                                                                m_resolutionContext);
            }
            ...
            template<class T>
            std::vector<std::shared_ptr<T> > resolveAll(const TypeAliasKey &typeAliasKey,
                                                        const std::vector<std::shared_ptr<RegistrationContext> > &registrationContexts) {
            ...
            bool tryGetRegistrations(const TypeAliasKey &typeAliasKey,
                                     std::vector<std::shared_ptr<RegistrationContext> > &registrationContexts) const {
            ...
            std::shared_ptr<T> resolveIfTypeCanBeRegistered() {
                if (!Behavior::isRuntimeRegistrationEnabled() ||
                    !tryToRegisterType<T>(*m_registrationScope, Traits::HasAutowireableConstructor<T>()))
                    HYPODERMIC_THROW_RESOLUTION_EXCEPTION(
                            "Unable to resolve " << Utils::getMetaTypeInfo<T>().fullyQualifiedName());
            ...
            bool tryToRegisterType(IRegistrationScope &scope, std::true_type /* T has autowireable constructor */) {
                ...
                scope.addRegistration(m_runtimeRegistrationBuilder->build
                        (
                                ...
                                [factory](const IRegistration &r, IResolutionContext &c) {
                                    return std::static_pointer_cast<void>(factory(r, c));
                                }
                        ));
