...
#include "ActivationResult.hpp"
#include "ActivationException.hpp"
#include "ActivatedRegistrationInfo.hpp"
#include "ActivationRegistry.hpp"
#include "CircularDependencyException.hpp"
#include "DependencyActivationException.hpp"
#include "InstanceAlreadyActivatingException.hpp"
#include "IRegistrationActivator.hpp"
#include "IResolutionContainer.hpp"
#include "Log.hpp"
#include "PointerAlignment.hpp"
...
namespace Hypodermic {
    ...
    class ResolutionContainer : public IResolutionContainer {
        public:
            std::shared_ptr<void>
            getOrCreateComponent(const TypeAliasKey &typeAliasKey, const std::shared_ptr<IRegistration> &registration,
                                 ResolutionContext &resolutionContext) override {
                ...
                if (isActivating(registration, resolutionContext.resolutionStack())) {
                    ...
                    HYPODERMIC_THROW_INSTANCE_ALREADY_ACTIVATING_EXCEPTION(
                            "Already activating " << prettyPrintResolutionInfo(*registration, typeAliasKey));
                }
                ...
                try {
                    ...
                    HYPODERMIC_LOG_ERROR("Circular dependency detected while activating "
                                                 << registration->instanceType().fullyQualifiedName() << ": "
                                                 << ex.what());

                    logAndClearResolutionStack(resolutionStack);

                    HYPODERMIC_THROW_CIRCULAR_DEPENDENCY_EXCEPTION("Circular dependency detected while activating "
                                                                           << registration->instanceType().fullyQualifiedName()
                                                                           << ": " << ex.what());
                }
                catch (std::exception &ex) {
                    HYPODERMIC_LOG_ERROR(
                            "Unable to activate instance of type " << registration->instanceType().fullyQualifiedName()
                                                                   << ": " << ex.what());

                    logAndClearResolutionStack(resolutionStack);

                    HYPODERMIC_THROW_ACTIVATION_EXCEPTION(
                            "Unable to activate instance of type " << registration->instanceType().fullyQualifiedName()
                                                                   << ": " << ex.what());
                }
        ...
        private:
            ActivationResult
            getOrActivateComponent(const TypeAliasKey &typeAliasKey, const std::shared_ptr<IRegistration> &registration,
                ...
                activationResult.alignedInstance = registry.getOrCreateAlignedInstance(typeAliasKey,
                                                                                       registration->typeAliases());
            ...
            ActivationResult activateComponentAndRegisterActivatedInstance(const TypeAliasKey &typeAliasKey,
                ...
                if (activationResult.activated)
                    m_activationRegistriesByRegistration.insert(
                            std::make_pair(registration, ActivationRegistry(activationResult.activatedInstance)));

                return activationResult;
            }

            static ActivationResult
            activateComponent(const TypeAliasKey &typeAliasKey, const std::shared_ptr<IRegistration> &registration,
                              IResolutionContext &resolutionContext) {
                ActivationResult activationResult;
                activationResult.activatedInstance = activateInstance(registration, resolutionContext);
                activationResult.alignedInstance = Utils::getAlignedPointer(activationResult.activatedInstance,
                                                                            typeAliasKey, registration->typeAliases());
                activationResult.activated = activationResult.activatedInstance != nullptr;

                return activationResult;
            }

            static std::shared_ptr<void> activateInstance(const std::shared_ptr<IRegistration> &registration,
                                                          IResolutionContext &resolutionContext) {
                auto &&instance = registration->activator().activate(resolutionContext);
                if (instance == nullptr)
                    HYPODERMIC_THROW_ACTIVATION_EXCEPTION("Unable to activate instance of type "
                                                                  << registration->instanceType().fullyQualifiedName());

                return instance;
            }

            static void notifyActivatedRegistrations(ResolutionContext::ActivatedRegistrations &activatedRegistrations,
            ...
            static std::string
            prettyPrintResolutionInfo(const IRegistration &registration, const TypeAliasKey &typeAliasKey) {
                std::stringstream stream;

                if (registration.instanceType() == typeAliasKey.typeAlias().typeInfo())
                    stream << typeAliasKey.typeAlias().toString();
                else
                    stream << typeAliasKey.typeAlias().toString() << " (base of "
                           << registration.instanceType().fullyQualifiedName() << ")";

                return stream.str();
            }

            static bool isActivating(const std::shared_ptr<IRegistration> &registration,
                                     const ResolutionContext::ResolutionStack &resolutionStack) {
            ...
            static void logAndClearResolutionStack(ResolutionContext::ResolutionStack &resolutionStack) {
                ...
                do {
                    ...
                    if (registration.instanceType() == typeAliasKey.typeAlias().typeInfo())
                        ...
                        stream << resolutionInfo.registration()->instanceType().fullyQualifiedName() << " as "
                               << resolutionInfo.typeAliasKey().typeAlias().toString();
