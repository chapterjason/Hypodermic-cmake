...
namespace Hypodermic {
    ...
    class ActivationRegistry {
        public:
            ...
            std::shared_ptr<void>
            getOrCreateAlignedInstance(const TypeAliasKey &typeAliasKey, const TypeAliases &typeAliases) {
                auto it = m_alignedInstances.find(typeAliasKey);
                if (it == std::end(m_alignedInstances))
                    it = m_alignedInstances.insert(std::make_pair(typeAliasKey,
                                                                  Utils::getAlignedPointer(m_activatedInstance,
                                                                                           typeAliasKey,
                                                                                           typeAliases))).first;
