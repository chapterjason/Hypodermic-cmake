...
#include "ContainerInstanceRegistration.hpp"
#include "IRegistrationScope.hpp"
#include "Log.hpp"
#include "RegistrationDescriptorBuilder.hpp"
#include "RuntimeRegistrationBuilder.hpp"
...
namespace Hypodermic {
    ...
    class ContainerBuilder {
        ...
        public:
            ...
            template<class T>
            typename RegistrationDescriptorBuilder::ForProvidedInstance<T>::Type &
            registerInstance(const std::shared_ptr<T> &instance) {
            ...
            typename RegistrationDescriptorBuilder::ForProvidedInstanceFactory<TCallable>::Type &
            registerInstanceFactory(const TCallable &instanceFactory) {
            ...
            void addRegistrations(const ContainerBuilder &builder) {
                m_registrationDescriptors.insert(std::end(m_registrationDescriptors),
                                                 std::begin(builder.m_registrationDescriptors),
                                                 std::end(builder.m_registrationDescriptors));
        ...
        private:
            void registerContainerInstance(const std::shared_ptr<Container> &container,
                                           const std::shared_ptr<IRegistrationRegistry> &scope) const {
            ...
            createAndRegisterContainerInstance(const std::shared_ptr<IRegistrationScope> &scope) const {
            ...
            finalizeRegistration(const std::shared_ptr<TRegistrationDescriptor> &registrationDescriptor) {
            ...
            void listenToRegistrationDescriptorUpdates(
                    const std::shared_ptr<IRegistrationDescriptor> &registrationDescriptor) {
                std::weak_ptr<IRegistrationDescriptor> weakDescriptor = registrationDescriptor;

                registrationDescriptor->registrationDescriptorUpdated().connect(
                        [this, weakDescriptor](const std::shared_ptr<IRegistrationDescriptor> &x) {
        ...
        private:
            ...
            std::unordered_map<std::shared_ptr<IRegistrationDescriptor>, std::function<void(
                    IRegistrationRegistry &)> > m_buildActions;
    };
