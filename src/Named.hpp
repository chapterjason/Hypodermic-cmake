        ...
        class Named {
            private:
                ...
                struct EnforceBaseOf {
                    static_assert(std::is_base_of<TBase, T>::value && !std::is_same<TBase, T>::value,
                                  "TBase should be a base of T");
                ...
                struct EnforceBaseNotAlreadyRegistered {
                    static_assert(!TDescriptorInfo::template IsBaseRegistered<TBase>::value,
                                  "TBase is already registered for instance T");
            ...
            public:
                ...
                ::type named(const std::string &name) {
                    ...
                    auto descriptor = static_cast< TDescriptor * >(this);
                    descriptor->addTypeIfMissing(createKeyForNamedType<TBase>(name),
                                                 [](const std::shared_ptr<void> &x) {
                                                     auto instanceDynamicType = std::static_pointer_cast<InstanceType>(
                                                             x);
                                                     auto instanceStaticType = std::static_pointer_cast<TBase>(
                                                             instanceDynamicType);
