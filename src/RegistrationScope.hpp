...
#include "RegistrationContext.hpp"
    ...
    class RegistrationScope : public IRegistrationScope {
        public:
            ...
            bool tryGetRegistrations(const TypeAliasKey &typeAliasKey,
                                     std::vector<std::shared_ptr<RegistrationContext> > &registrationContexts) const override {
            ...
            void addRegistrationContext(const std::shared_ptr<RegistrationContext> &registrationContext) override {
                if (registrationContext->registration()->typeAliases().empty()) {
                    addRegistrationContext(createKeyForType(registrationContext->registration()->instanceType()),
                                           registrationContext);
                    return;
                }
        ...
        private:
            void addRegistration(const TypeAliasKey &typeAliasKey, const std::shared_ptr<IRegistration> &registration) {
                addRegistrationContext(typeAliasKey,
                                       std::make_shared<RegistrationContext>(m_resolutionContainer, registration));
            }

            void addRegistrationContext(const TypeAliasKey &typeAliasKey,
                                        const std::shared_ptr<RegistrationContext> &registrationContext) {
                if (registrationContext->registration()->isFallback()) {
                    addRegistrationContext(m_fallbackRegistrationContextsByBaseTypes, typeAliasKey,
                                           registrationContext);
                } else {
            ...
            void addRegistrationContext(TRegistrationContextsByBaseTypes &contextsByBaseTypes,
                ...
                if (it == std::end(contextsByBaseTypes))
                    it = contextsByBaseTypes.insert(
                            std::make_pair(typeAliasKey, std::vector<std::shared_ptr<RegistrationContext> >())).first;
            ...
            template<class TRegistrationContextsByBaseTypes>
            void
            copyRegistrationContextsToRegistrationScope(const TRegistrationContextsByBaseTypes &contextsByBaseTypes,
                                                        IRegistrationScope &other) const {
