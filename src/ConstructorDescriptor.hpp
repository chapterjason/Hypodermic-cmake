...
#include "ConstructorTypologyDeducer.hpp"
#include "IsSupportedArgument.hpp"
    ...
    namespace Traits {
        ...
        namespace Details {
            ...
            struct ArgumentResolverInvoker {
                    explicit ArgumentResolverInvoker(const IRegistration &registration,
                                                     IResolutionContext &resolutionContext)
                            : m_registration(registration), m_resolutionContext(resolutionContext) {
                    }

                    template<class T, class = typename std::enable_if<!std::is_convertible<TParent, T>::value &&
                                                                      IsSupportedArgument<typename std::decay<T>::type>::value>::type>
                    operator T() {
                        return ArgumentResolver<typename std::decay<T>::type>::template resolve(m_registration,
                                                                                                m_resolutionContext);
                    }
            ...
            struct ConstructorDescriptor<T, Utils::ArgumentPack<TAnyArgument...> > {
                static std::function<std::shared_ptr<T>(const IRegistration &, IResolutionContext &)> describe() {
                    return [](const IRegistration &registration, IResolutionContext &resolutionContext) {
                        return std::make_shared<T>(ArgumentResolverInvoker<typename TAnyArgument::Type>(registration,
                                                                                                        resolutionContext)...);
                    };
                }
            };
