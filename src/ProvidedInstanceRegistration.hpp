...
#include "ProvidedInstanceRegistrationActivator.hpp"
    ...
    class ProvidedInstanceRegistration : public IRegistration {
        public:
            ProvidedInstanceRegistration(const std::shared_ptr<T> &instance,
                                         ...
                                         bool isFallback)
                    : m_activator(*this, instance), m_instanceType(Utils::getMetaTypeInfo<T>()),
                      m_typeAliases(typeAliases), m_isFallback(isFallback) {
            }
